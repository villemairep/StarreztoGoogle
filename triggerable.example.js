...
/**
 * Set up any number of triggerable functions. A single function can contain one
 * or more calls to updateGoogleSheetFromStarRez(). For instance, if you want to
 * run several reports daily at the same time, you could combine the calls into
 * one function, then trigger the function.
 *
 ...
 *
 * The updateExample function can be used as a template to copy/paste additional
 * functions that will be triggered individually.
 */
...
function updateExample() {
  ...
  var options = {
    ...
    "requestBody": {"Testing": 1},

    /**
     * Optional: Load report data into a specific sheet (by index or name)
     * within the spreadsheet. Defaults to first sheet, if not specified.
     */
    "sheet": "TestSheet",

    /**
     * Optional: Specify an error message to display when the report has no
     * results.
     */
  ...
  /**
   * Note that you can make related calls by just changing the options that are
